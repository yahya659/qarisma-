...
export default function SupportCategories() {
  const categories = [
    {
      ...
      color: "bg-[#F178B6]", // الوردي الغامق
      icon: "🚀", // يمكنك استبدالها بـ <img src="/rocket.png" />
    },
    {
      title: "Account &password",
      color: "bg-[#5D5FEF]", // البنفسجي المميز
      icon: "👤",
  ...
  return (
    <section className="w-full py-20 bg-[#BD9FA973] flex justify-center items-center px-6">
      ...
      <div className="max-w-7xl w-full grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
        ...
        {categories.map((item, index) => (
          <div
            key={index}
            className={`relative flex flex-col items-center justify-center gap-8 p-12   transition-all duration-300 hover:-translate-y-2 ${item.color} ${
              item.active
          ...
          >
            ...
            <h3 className="text-white text-[32px]   font-bold text-center leading-tight drop-shadow-sm">
              {item.title}
            </h3>
          </div>
        ))}
